/**
 * l3-roles/role-loader.ts 单测（P1.1.6 验收）。
 *
 * 验证：
 * - R6 YAML → 可用的 RoleProfile
...
afterEach(() => {
  // 测试创建的临时目录在各用例内清理；这里兜底
})

describe('compileRoleToProvider 字段映射', () => {
  it('R6 YAML → RoleProfile：字段完整映射', () => {
    const { dir, cleanup } = tempSkillsDir()
    try {
      const profile = compileRoleToProvider(validRole, { skillsDir: dir })
      ...
      // private → inheritsParentContext=false
  ...
  it('shared → inheritsParentContext=true', () => {
    const { dir, cleanup } = tempSkillsDir()
    try {
      const profile = compileRoleToProvider({ ...validRole, memory_scope: 'shared' }, { skillsDir: dir })
  ...
  it('绝对路径 system_prompt_ref 可用', () => {
    ...
    try {
      ...
      const profile = compileRoleToProvider({ ...validRole, system_prompt_ref: absRef }, { skillsDir: dir })
      expect(profile.persona).toContain('TypeScript 开发者')
    } finally {
      cleanup()
    }
...
describe('路径逃逸防护', () => {
  it('拒绝逃逸 skillsDir 的相对路径', () => {
    ...
    try {
      ...
      expect(() => compileRoleToProvider(escaping, { skillsDir: dir })).toThrow(RoleLoadError)
      expect(() => compileRoleToProvider(escaping, { skillsDir: dir })).toThrow(/不允许逃逸/)
    } finally {
      cleanup()
    }
  })
...
describe('skill 文件错误处理', () => {
  it('skill 文件不存在抛 RoleLoadError', () => {
    ...
    try {
      expect(() =>
        compileRoleToProvider({ ...validRole, system_prompt_ref: 'missing/SKILL.md' }, { skillsDir: dir }),
      ).toThrow(RoleLoadError)
      expect(() =>
        compileRoleToProvider({ ...validRole, system_prompt_ref: 'missing/SKILL.md' }, { skillsDir: dir }),
      ).toThrow(/skill 文件不存在/)
    } finally {
...
`,
      ...
      const profiles = compileRoleDirectory(rolesDir, { skillsDir: dir })

...
| 项 | 期望值 | 实测值 | 结果 |
|---|---|---|---|
| DSH 版本 | 0.1.5-rc2（文档标注） | **0.1.5-rc.2** | ⚠️ 差异（见 §二） |
...
| 项 | 内容 |
|---|---|
| 文档标注 | `1-DevTask.md` / `README.md` 均写 **0.1.5-rc2** |
| 实测值 | `dsh --version` → **0.1.5-rc.2** |
| 包版本 | `@deepseek-ai/dsh` = 0.1.5-rc.2；`@deepseek-ai/dsh-subagent` = 0.1.5-rc.2；`@deepseek-ai/cordis` = 4.0.2 |
| 结论 | 以实测 0.1.5-rc.2 为准；脚手架 peerDependencies 版本按实测线声明；若与文档 0.1.5-rc2 存在 API 差异，以官方源码为准并记录 |
...
- [x] `dsh --version` → 0.1.5-rc.2（与文档 0.1.5-rc2 有差异，已记录）
